...
exports.getTechPortAnalytics = exports.getTechPortCategories = exports.fetchTechPortProject = exports.fetchTechPortProjects = exports.fetchHistoricMarsWeatherData = exports.getMultiPlanetaryDashboard = exports.checkNASAApiHealth = exports.fetchPerseveranceWeatherData = exports.getMostActiveRover = exports.fetchRoverManifest = exports.fetchMarsRoverPhotos = exports.fetchAPODData = void 0;
...
const nasaApiClient = axios_1.default.create({
...
const fetchAPODData = async (date) => {
    ...
    const response = await nasaApiClient.get(nasaConfig.endpoints.apod, { params });
    return response.data;
};
exports.fetchAPODData = fetchAPODData;
// Fetch Mars Rover photos with dynamic rover support
const fetchMarsRoverPhotos = async (rover = 'curiosity', sol) => {
    try {
        ...
        const response = await nasaApiClient.get(endpoint, { params });
...
exports.fetchMarsRoverPhotos = fetchMarsRoverPhotos;
...
exports.fetchRoverManifest = fetchRoverManifest;
// Get the most recently active rover
const getMostActiveRover = async () => {
    try {
        const manifests = await Promise.allSettled(nasaConfig.rovers.map(async (rover) => {
            const manifest = await (0, exports.fetchRoverManifest)(rover);
            return {
                rover,
        ...
        const mostActive = activeRovers.reduce((prev, current) => new Date(current.maxDate) > new Date(prev.maxDate) ? current : prev);
...
exports.getMostActiveRover = getMostActiveRover;
// Fetch Mars weather data from available NASA sources
const fetchPerseveranceWeatherData = async () => {
    try {
...
exports.fetchPerseveranceWeatherData = fetchPerseveranceWeatherData;
...
const fetchInSightWeatherData = async () => {
    try {
...
const fetchMSLWeatherData = async () => {
    try {
        const response = await axios_1.default.get(nasaConfig.endpoints.marsWeatherService, {
            timeout: nasaConfig.timeout
        });
        return response.data;
    }
...
const fetchMAASWeatherData = async () => {
    try {
        const response = await axios_1.default.get(nasaConfig.endpoints.maasWeather, {
            timeout: nasaConfig.timeout
        });
        return response.data;
    }
...
const convertInSightToPerseveranceFormat = (insightData) => {
    const currentSol = Math.max(...insightData.sol_keys.map((sol) => parseInt(sol)));
...
const convertMSLToPerseveranceFormat = (mslData) => {
...
const convertMAASToPersevaeranceFormat = (maasData) => {
...
const getWindDegrees = (direction) => {
    const windMap = {
...
const generateRealisticMarsWeather = () => {
...
const checkNASAApiHealth = async () => {
    try {
...
exports.checkNASAApiHealth = checkNASAApiHealth;
// Multi-Planetary Dashboard Data Generator
const getMultiPlanetaryDashboard = async () => {
    const currentDate = new Date();
    const timestamp = currentDate.toISOString();
    // Calculate days difference
    const daysDiff = (date1, date2) => Math.floor((date1.getTime() - date2.getTime()) / (1000 * 60 * 60 * 24));
    const planetsData = [
        {
            id: 'mars',
            name: 'Mars',
    ...
    const total_active_missions = planetsData.reduce((acc, p) => acc + p.active_missions.length, 0);
    return {
        planets: planetsData,
...
exports.getMultiPlanetaryDashboard = getMultiPlanetaryDashboard;
// Historic Mars Weather Data Aggregator
// This function fetches either real NASA data or simulated data (no hybrid)
const fetchHistoricMarsWeatherData = async (params) => {
...
exports.fetchHistoricMarsWeatherData = fetchHistoricMarsWeatherData;
// Enhanced InSight data fetching with retry logic
const fetchInSightWeatherDataWithRetry = async () => {
    const endpoints = [
...
const processInsightDataForCharts = (insightData) => {
    const missionInfo = {
        ...
        earth_dates: {
            start: insightData[insightData.sol_keys[0]]?.First_UTC?.split('T')[0] || '2018-11-26',
            end: insightData[insightData.sol_keys[insightData.sol_keys.length - 1]]?.Last_UTC?.split('T')[0] || '2022-12-15'
        },
    ...
    insightData.sol_keys.forEach((sol) => {
        const solData = insightData[sol];
...
const generateHistoricalSimulationData = (params) => {
...
const fetchTechPortProjects = async (params = {}) => {
    try {
        const response = await nasaApiClient.get(nasaConfig.endpoints.techport, {
            params: {
                ...params
            }
        });
...
exports.fetchTechPortProjects = fetchTechPortProjects;
// Fetch a single TechPort project by ID
const fetchTechPortProject = async (projectId) => {
    try {
        const response = await axios_1.default.get(`https://techport.nasa.gov/api/projects/${projectId}`, {
            timeout: 10000
        });
        return response.data;
    }
...
exports.fetchTechPortProject = fetchTechPortProject;
// Aggregate and return a list of technology categories
const getTechPortCategories = async () => {
    try {
        // In a real scenario, this might be a separate endpoint or aggregated from projects
        const projectsResponse = (await (0, exports.fetchTechPortProjects)({ limit: 500 }));
...
exports.getTechPortCategories = getTechPortCategories;
const getTechPortAnalytics = async () => {
    try {
        const projectsResponse = (await (0, exports.fetchTechPortProjects)({
            limit: 1000
        }));
        const statusCounts = projectsResponse.projects.reduce((acc, project) => {
        ...
        const categoryCounts = projectsResponse.projects.reduce((acc, project) => {
            project.technologyAreas.forEach(area => {
...
exports.getTechPortAnalytics = getTechPortAnalytics;
